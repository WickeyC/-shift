...
const filterAudioProducts = importedProducts.filter((product) => {
  return product.type === 'AUDIO';
});

const AudioProductsPage = () => {
  const [products, setProducts] = useState(filterAudioProducts);
  ...
  const handleSearchSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (searchTerm.trim() === '') {
      ...
      setSearchResultText('');
      setProducts(filterAudioProducts);
      setSearchTerm('');
    } else {
      ...
      const searchedProducts = filterAudioProducts.filter((product) => {
  ...
  return (
    <>
      <Head>
        <title>Audio - Shift</title>
      </Head>
      <Header />
      <main>
        <CategoryHeader>
          <div className="container">
            <Title>Explore Audio</Title>
...
export default AudioProductsPage;
