...
const filterPhoneProducts = importedProducts.filter((product) => {
  return product.type === 'PHONE';
});

const PhoneProductsPage = () => {
  const [products, setProducts] = useState(filterPhoneProducts);
  ...
  const handleSearchSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (searchTerm.trim() === '') {
      ...
      setSearchResultText('');
      setProducts(filterPhoneProducts);
      setSearchTerm('');
    } else {
      ...
      const searchedProducts = filterPhoneProducts.filter((product) => {
  ...
  return (
    <>
      <Head>
        <title>Phones - Shift</title>
      </Head>
      <Header />
      <main>
        <CategoryHeader>
          <div className="container">
            <Title>Explore Phones</Title>
